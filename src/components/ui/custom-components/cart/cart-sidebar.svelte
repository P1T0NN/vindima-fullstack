<script lang="ts">
	...
	function checkout() {
		cart.close();
		appGoto(UNPROTECTED_PAGE_ENDPOINTS.CHECKOUT);
	}
...
<Sheet.Root bind:open={cart.isOpen}>
	<Sheet.Content side="right" class="w-full gap-0 bg-background p-0 sm:max-w-105">
		<Sheet.Header class="sr-only">
			<Sheet.Title>Carrito</Sheet.Title>
			<Sheet.Description>Contenido actual del carrito.</Sheet.Description>
		</Sheet.Header>
		...
		<div class="min-h-0 flex-1 overflow-y-auto px-4">
			...
			{:else if showEmpty}
				<div class="flex h-full flex-col items-center justify-center gap-4 py-16 text-center">
					<span class="icon-[lucide--shopping-bag] size-12 text-muted-foreground/40"></span>
