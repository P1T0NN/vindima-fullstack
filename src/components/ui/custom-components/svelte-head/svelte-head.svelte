<script lang="ts">
	...
	// when `PUBLIC_ORIGIN` is unset (e.g. preview deploys without env configured).
	 ...
	 * may declare it — the home page, matched by route id rather than by pathname so a moved
	 * root never silently drops it. Every value comes from `COMPANY_DATA`.
	 */
