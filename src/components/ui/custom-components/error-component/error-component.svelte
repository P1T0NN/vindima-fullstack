<script lang="ts">
	...
	import { TryAgainErrorButton } from '@/components/ui/custom-components/try-again-error-button/index.js';
	...
	>
		{#if showIcon}
			<div
				...
				aria-hidden="true"
			>
				<span class="icon-[lucide--triangle-alert] size-6" ></span>
			</div>
		{/if}
			...
			>
				{#if showIcon}
					<div
						...
						aria-hidden="true"
					>
						<span class={['icon-[lucide--triangle-alert]', variant === 'panel' ? 'size-6' : 'size-5']} ></span>
					</div>
				{/if}
