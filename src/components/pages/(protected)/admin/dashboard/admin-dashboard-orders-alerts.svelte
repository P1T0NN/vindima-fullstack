...
{:else}
	<div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
		{#each alerts as alert (alert.href)}
			...
			>
				<span class={alert.icon + ' size-5 shrink-0 text-accent'} aria-hidden="true"></span>
				...
				<span
					class="icon-[lucide--arrow-right] size-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5"
					aria-hidden="true"
				></span>
			</a>
		{/each}
