 ...
 *
 * Convex returns `message: { key, params? }` envelopes (never display text), and zod's
 * default error map emits the same kind of code; this map renders them to Spanish on the
 * client. Unknown keys fall back to the key literal (visible in dev — by design).
 *
 * Lives in `shared/` so both runtimes can reference the key names, but only the frontend
 * (`@/features/validations/utils/translateFromBackend`) ever reads the text out of it.
 * `{param}` placeholders are filled by {@link formatMessage}.
 */
export const BACKEND_MESSAGES: Record<string, string> = {
	...
	// Zod default error map (`shared/features/validations/utils/mapDefaultValidationErrors.ts`).
	// Generic, violation-shaped copy — the fallback when a path has no bespoke sentence.
	'ValidationMessages.Default.required': 'Este campo es obligatorio.',
	'ValidationMessages.Default.invalidValue': 'Revisa este campo.',
	'ValidationMessages.Default.notANumber': 'Escribe un número.',
	'ValidationMessages.Default.invalidDate': 'Escribe una fecha válida.',
	'ValidationMessages.Default.invalidEmail': 'Escribe un correo electrónico válido.',
	'ValidationMessages.Default.invalidUrl': 'Escribe un enlace válido.',
	'ValidationMessages.Default.invalidChoice': 'Selecciona una opción válida.',
	'ValidationMessages.Default.textTooShort': 'Debe tener al menos {min} caracteres.',
	'ValidationMessages.Default.textTooLong': 'El valor es demasiado largo (máx. {max} caracteres).',
	'ValidationMessages.Default.tooFewItems': 'Agrega al menos {min}.',
	'ValidationMessages.Default.tooManyItems': 'Agrega como máximo {max}.',
	'ValidationMessages.Default.numberNotNegative': 'No puede ser un número negativo.',
	'ValidationMessages.Default.numberTooSmall': 'Escribe {min} o más.',
	'ValidationMessages.Default.numberTooBig': 'Escribe {max} o menos.',

	// Bespoke per-path copy (`VALIDATION_PATH_MESSAGE_KEYS`) — checkout/contact fields.
	'ValidationMessages.Path.contactName': 'Escribe tu nombre.',
	'ValidationMessages.Path.contactEmail': 'Escribe un correo electrónico válido.',
	'ValidationMessages.Path.contactPhone': 'Escribe un teléfono válido.',
	'ValidationMessages.Path.addressLine1': 'Escribe tu calle y número.',
	'ValidationMessages.Path.addressCity': 'Escribe tu ciudad.',
	'ValidationMessages.Path.addressPostcode': 'Escribe tu código postal.',
	'ValidationMessages.Path.addressCountry': 'Escribe tu país.',
	'ValidationMessages.Path.cartEmpty': 'Tu carrito está vacío.',

	// Bespoke auth-schema messages (bare catalog keys carried on the schema itself).
	'ValidationMessages.Auth.codeFormat': `Usa el código de ${AUTH_DATA.OTP_LENGTH} dígitos de tu correo.`,
	'ValidationMessages.Auth.passwordTooCommon':
		'Esa contraseña es demasiado común. Elige una más segura.',
	'ValidationMessages.Auth.passwordsMustMatch': 'Las contraseñas deben coincidir.'
};
