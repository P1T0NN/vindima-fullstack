...
```
fetchOptimized/
├── fetchOptimized.ts      the list factory (where | search | union | aggregate | resolve)
...
**Which factory?** Paginated _list_ for `ConvexDataTable` / `ConvexDataList` →
...
> **Not ported into this project.** This guide comes from the upstream template and mentions a
> few things that do not exist here: the URL-driven table helpers (`listHref`, `sortHref`,
> `pageHref`), `ConvexInfiniteList`, the `search.remote.ts` IP-limited bridge, and the
> `fetchTestRows*` demo endpoints. Treat those passages as "what porting would look like", not
> as available API — see `docs/GeneralSystemDesignRule.md § LIST & PAGINATION MECHANISMS`
> for the mechanisms this project actually has.
...
| `'offset'` (scan)      | O(rows)     | exact\*    | capped       | any change to the matched set reruns    |
| `'offset'` + aggregate | O(log n)    | exact      | unlimited    | any change to the matched set reruns    |

Default `'cursor'` — always cheap as the table grows. Pick `'offset'` when you need
page-number jumps or an accurate total: the plain scan form is fine for bounded sets
(\*past `OFFSET_SCAN_LIMIT` it degrades to `totalCount: null` + prev/next), and pairing
it with an `aggregate` builder removes the bound entirely — exact totals and O(log n)
jumps to any page at any table size (§ `aggregate` below).
...
5. **`aggregate`** — an offset surface (page numbers, exact totals) whose matched set
   grows without bound. Costs write-path wiring (one counter per surface, kept in sync
   by triggers), so reach for it only when the scan cap actually bites.
6. **`resolve`** — the escape hatch: you own the data source, the factory owns the
...
be schema-declared `filterFields` of the search index, not arbitrary doc keys. Empty query
string returns no rows by Convex semantics.

Requires `strategy: 'cursor'` on the requests it serves — Convex search indexes are
paginate-only. A static `strategy: 'offset'` alongside a `search` builder throws at factory
build time; a `strategy` **function** is checked per request instead, which is what lets one
query serve browse-with-page-numbers and search from the same endpoint (§ Public,
...
- **Offset mode:** N bounded collects → dedupe by `_id` → sort by `sortBy` → slice.
  ...
  **Bounded-only BY DESIGN — there is no aggregate escape hatch for union+offset**, unlike
  plain offset. The blocker is fundamental, not unbuilt: aggregate counters cannot dedupe
  across specs (a row matching two specs is counted twice, so a `countBatch` total would be
  a wrong number rendered with confidence), and a deduped page jump across merged
  namespaces has no O(log n) form. If a union surface outgrows the scan cap, that is the
  signal to use **cursor mode** (O(perPage · k) forever) or to denormalize a single access
  key onto the row so plain offset + aggregate applies.
...
(defined in `convex/counters.ts`, kept in sync by write-path triggers via
...
per surface, maintained on every write.

```ts
export const fetchProducts = fetchOptimized({
	table: 'products',
	strategy: 'offset',
	aggregate: () => ({ aggregate: aggregateProductsByCreation })
});
```

Contract:

- **`strategy: 'offset'` only.** Cursor mode is already O(perPage) natively; a static
  cursor strategy (including the default) throws at build time.
- **The aggregate's `sortKey` IS the sort order.** No index is consulted for the slice —
  define one aggregate per sort field a surface offers. `order: 'desc'` is served from
  the same aggregate (positions counted from the end).
- **Filters go through the aggregate too**: exact-match facets via `namespace`, ranges
  via key `bounds`. A builder that can't express the request returns `null` and the
  request falls through to the capped-scan offset path — the standard shape is
  "aggregate for the unbounded browse, scan for small filtered sets"
  (see `fetchTestRowsCatalog`).
- **Write paths must not bypass the triggers.** All mutations import from
  `convex/functions.ts`; a raw `_generated/server` mutation writing a counted table
  silently drifts the count. Backfill once after adding an aggregate to an existing
  table (`counters:backfillOrderCounts` is the pattern).
- Per-request cost: 1 count + 1 batched offset lookup + `numItems` gets by id — then
  `enrich` as usual.
...
5. **Offset mode kept honest.** The scan form is O(rows) with a hard cap and a
   `totalCount: null` degradation instead of a wrong number; the `aggregate` form
   removes the bound at O(log n). Either way the factory owns
   totalCount/slice/isDone/page-clamp, so no call site re-derives them.

## Public, server-rendered listing (browse + search + `?page=N`)

The catalog / docs-index / job-board shape: filters, free-text search, page numbers in the
URL, results in the initial HTML. It is **one query**, not two.

`strategy` takes a function, so browse and search can pick different pagination modes on the
same endpoint. Keep the index-selection logic in one inferred options object — hoisting it
into a shared `const` erases the factory's generics and silently stops checking index names
against the schema.
...
export const fetchProducts = fetchOptimized({
	...
	// Search can only ever be cursor-paginated; browse gets page numbers + totalCount.
	strategy: (args) => (args.q ? 'cursor' : 'offset'),
	search: (_ctx, args) =>
		args.q ? { index: 'search_name', searchField: 'name', query: args.q } : null,
	where: (_ctx, args) =>
		args.q ? null : { index: 'by_category', eq: { category: args.category ?? 'all' } }
});
```

The route loader owns the URL; the component just renders what it is handed.

```ts
// +page.server.ts
import { readPage } from '@/features/pagination/utils/listUrlState.js';

export const load = async ({ url }) => {
	const q = url.searchParams.get('q') ?? undefined;
	const result = await createConvexHttpClient().query(api.tables.products.fetchProducts, {
		q,
		category: url.searchParams.get('category') ?? undefined,
		page: readPage(url),
		paginationOpts: { numItems: 20, cursor: null }
	});
	return { result, isSearch: Boolean(q) };
};
```

```svelte
<!-- +page.svelte — DataList directly, no Convex wrapper: no subscription, no client fetch -->
<DataList
	items={data.result.page}
	{item}
	page={readPage(page.url)}
	totalPages={data.result.totalCount ? Math.ceil(data.result.totalCount / 20) : undefined}
	canGoNext={!data.result.isDone}
	pageHref={(p) => listHref(page.url, { page: p })}
	showPagination
	hasResult
/>
```

`pageHref` switches `PaginatedData` to real `<a href>` links with `rel="prev"`/`rel="next"`
and an elided page-number window — crawlable, middle-clickable, working with JavaScript off.
`listHref` drops the page whenever a filter changes and never emits `?page=1`.

**What you do not get, and cannot:** page numbers during search. `totalCount` is `null` in
cursor mode, `totalPages` goes `undefined`, and the paginator falls back to prev/next. That
is Convex's constraint (search indexes are paginate-only), not the template's — the surface
has to absorb it. Everything else is identical between the two modes.

Use `ConvexDataList` / `ConvexDataTable` instead when the page is authenticated, has no
loader, and wants a live subscription. Use the plain `DataList` / `DataTable` + a loader for
anything public and server-rendered.
