 ...
 * - `draft`   — an unpaid online order; NOT a real order yet, so nothing reads this bucket
 ...
 *
 * Giving drafts their own namespace (rather than skipping the aggregate for them) is what keeps
 * every existing `insert` / `replaceOrInsert` / `delete` call site correct without a single
 * conditional: a draft settling is just a `draft → open` transition like any other.
 *
