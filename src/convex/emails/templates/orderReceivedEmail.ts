 ...
 * to exist is the "what happens next" line: finish payment with a CTA to `paymentUrl`.
 * The receipt (O2) is the first email for the current online-only flow.
 *
...
export function orderReceivedEmail(order: Doc<'orders'>, paymentUrl?: string): EmailContent {
	...
	const nextLine = paymentUrl
		...
		: `Te avisaremos cuando esté ${order.delivery.kind === 'pickup' ? 'listo' : 'en camino'}.`;
